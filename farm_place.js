// kubejs/server_scripts/farm/farm_place.js

const SMALL_FARM_SIZE = 11
...
switch (name) {
    ...
    case "빨간 농장 생성권":
        player.runCommandSilent("//schem load red_small_farm")
        break

    case "노란 농장 생성권":
        player.runCommandSilent("//schem load yellow_small_farm")
        break

    case "연두 농장 생성권":
        player.runCommandSilent("//schem load lime_small_farm")
        break

    case "하늘 농장 생성권":
        player.runCommandSilent("//schem load light_blue_small_farm")
        break

    case "보라 농장 생성권":
        player.runCommandSilent("//schem load purple_small_farm")
        break

    case "핑크 농장 생성권":
        player.runCommandSilent("//schem load pink_small_farm")
        break

    case "흰색 농장 생성권":
        player.runCommandSilent("//schem load white_small_farm")
        break

    case "검은 농장 생성권":
        player.runCommandSilent("//schem load black_small_farm")
        break
    ...
    // 이미 농장이 있는지 검사
    if (hasFarmBorder(player, SMALL_FARM_SIZE)) {
        player.tell("§c이미 다른 농장이 있는 위치입니다.")
        return
    }

    player.tell("§a농장을 설치합니다.")
    player.tell("§7불러오는 Schem : " + schem)
